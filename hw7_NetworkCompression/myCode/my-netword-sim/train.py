...
parser.add_argument('--arch', default='simpleconv3', type=str,
...
if args.cuda:
    ...
    kwargs = {'run_workers': 1, 'pin_memory': True}
...
## 数据读取与预处理方法
data_dir = '/mnt/f/code/PyCharmProject/lihongyiLessonCode/LHY_ML2020_Codes/hw7_NetworkCompression/myCode/netword-sim/data/'
data_transforms = {
...
for epoch in range(args.start_epoch, args.epochs):
    if epoch in [args.epochs * 0.5, args.epochs * 0.7]:
        for param_group in optimizer.param_groups:
            param_group['lr'] *= 0.1

    pre_train = process(epoch, train_loader, istrain=True)
    pre1 = process(epoch, test_loader, istrain=False)
    is_best = pre1 > best_prec1
    best_prec1 = max(pre1, best_prec1)
